...
export default function ScanPage() {
  ...
  const handleScanResult = (result: string) => {
    ...
    // Expected format: http://192.168.1.100/c/ST-20250207-001
    const match = result.match(/\/c\/(ST-\d{8}-\d{3})/)
    if (match) {
      router.push(`/admin/customers/${match[1]}`)
    } else if (result.startsWith('ST-')) {
      // Direct display ID
      router.push(`/admin/customers/${result}`)
    } else {
