...
router.get('/live', authenticate, asyncHandler(async (req: AuthRequest, res: any) => {
  ...
  const cancelled = todayFlights.filter(f => f.status === 'CANCELLED' && f.cancellationReason !== 'FORFEIT' && !(f.notes && (f.notes as string).toLowerCase().includes('feragat')) && !(f.cancellationNote && (f.cancellationNote as string).toLowerCase().includes('feragat')));
...
router.post('/:id/cancel', authenticate, asyncHandler(async (req: AuthRequest, res: any) => {
  ...
  const updatedFlight = await prisma.$transaction(async (tx) => {
    ...
    if (preserveQueue) {
      // Kötü hava / müşteri iptal: pilot UNAVAILABLE'a geçer, geri al (0'ın altına düşürme)
      ...
      await tx.pilot.update({
        ...
        data: {
          ...
          roundCount: cancelPilot && cancelPilot.roundCount > 0 ? { decrement: 1 } : 0,
        },
      });
    } else {
